...
class FluidEdgeClassLike {

	private static $instance;

	private function __construct() {
		add_action('wp_enqueue_scripts', array( $this, 'enqueue_scripts'));
		add_action('wp_ajax_fluid_edge_like', array( $this, 'ajax'));
		add_action('wp_ajax_nopriv_fluid_edge_like', array( $this, 'ajax'));
	}
	...
	function enqueue_scripts() {
		wp_enqueue_script( 'edgtf-like', EDGE_ASSETS_ROOT . '/js/modules/plugins/like.js', 'jquery', '1.0', TRUE );
	...
	public function add_like() {
		...
		$title = esc_html__('Like this', 'fluid');
		if( isset($_COOKIE['edgtf-like_'. $post->ID]) ){
			$class = 'edgtf-like liked';
			$title = esc_html__('You already like this!', 'fluid');
		}
...
if (!function_exists('fluid_edge_create_like')) {
    function fluid_edge_create_like() {
        FluidEdgeClassLike::get_instance();
    }

    add_action('after_setup_theme', 'fluid_edge_create_like');
}