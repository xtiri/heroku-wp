...
class FluidEdgeClassFramework
{
    ...
    private function __construct() {
        $this->edgtOptions = FluidEdgeClassOptions::get_instance();
        $this->edgtMetaBoxes = FluidEdgeClassMetaBoxes::get_instance();
    }
	...
	public function setSkin(FluidEdgeClassSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class FluidEdgeClassSkinManager
 *
...
/**
 * Class FluidEdgeClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class FluidEdgeClassSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(fluid_edge_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.edgtSCIcon = '<?php echo fluid_edge_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class FluidEdgeClassOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $fluid_edge_global_options)) {
            return $fluid_edge_global_options[$key];
...
/*
   Class: FluidEdgeClassAdminPage
   A class that initializes Edge Admin Page
*/
class FluidEdgeClassAdminPage implements iFluidEdgeInterfaceLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new FluidEdgeClassFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iFluidEdgeInterfaceRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: FluidEdgeClassMetaBoxes
...
/*
   Class: FluidEdgeClassMetaBox
   A class that initializes Edge Meta Box
*/
class FluidEdgeClassMetaBox implements iFluidEdgeInterfaceLayoutNode {
	...
	public function setTitle($label) {
		...
		return $fluid_edge_global_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if (!function_exists( 'fluid_edge_init_framework_variable')) {
    function fluid_edge_init_framework_variable() {
        global $fluid_edge_global_Framework;

        $fluid_edge_global_Framework = FluidEdgeClassFramework::get_instance();
        $edgtSkinManager = new FluidEdgeClassSkinManager();
        $fluid_edge_global_Framework->setSkin($edgtSkinManager->getSkin());
    }

    add_action('fluid_edge_action_before_options_map', 'fluid_edge_init_framework_variable');
}
?>